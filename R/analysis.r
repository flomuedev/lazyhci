...
ano <- function(data, dv, participantCol, within.vars = NULL, between.vars = NULL, posthoc.adj="bonferroni", anova.type = 2, normality.test = shapiro.test, latex.digits.round = 2) {
  ...
  dv.col = which(colnames(data.aov) == dv)
  data.aov[,"Q"] <- data.aov[dv.col]
  
  ezString <- build_ez_string.internal("data.aov", dv = "Q", participantCol = participantCol, within.vars = within.vars, between.vars = between.vars, anova.type = anova.type)
